...
const faqItems = [
    {
      id: "01",
      question: "What metrics should I track to measure digital marketing success?",
      answer:
    ...
    {
      id: "02",
      question: "How do you ensure the digital strategies are aligned with the latest market trends?",
      answer:
    ...
    },
    {
      ...
      answer:
        "We measure ROI by tracking conversions, sales, and other key performance indicators that align with our clients’ business objectives.s",
    },
    {
      id: "04",
      question: "How do you ensure brand consistency across all digital channels?",
      answer:
...
export default function Component() {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-6xl font-bold mb-16 text-center tracking-wider">
        FREQUENTLY ASKED QUESTIONS
      </h1>
      <div className="w-full max-w-4xl space-y-6">
        {faqItems.map((item) => (
          <div key={item.id} className="border-b border-[#9ACD32] pb-6">
            <button
            ...
            >
              <span className="text-[#9ACD32] mr-8 text-3xl font-bold">
                {item.id}
              </span>
              <span className="text-2xl font-bold flex-grow tracking-wide group-hover:text-[#9ACD32] transition-colors duration-300">
                {item.question}
              </span>
              <ChevronDownIcon
                className={`w-8 h-8 text-[#9ACD32] transition-transform duration-300 ${
                  activeQuestion === item.id ? "transform rotate-180" : ""
                }`}
              />
            </button>
            {activeQuestion === item.id && (
              <div className="mt-4 text-gray-300 text-lg leading-relaxed pl-16">
                {item.answer}
              </div>
            )}
          </div>
